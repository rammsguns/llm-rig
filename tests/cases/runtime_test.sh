...
test_kernel_support_follows_the_documented_architecture_boundaries() {
  ...
  assert_eq "$(vllm_kernels 7.4)"  "int"            "below the FP8 Marlin floor"  || return 1
  assert_eq "$(vllm_kernels 7.5)"  "int-fp8-weight" "Turing: FP8 Marlin starts"   || return 1
...
test_turing_loads_fp8_weight_only_and_is_never_told_it_has_fp8_hardware() {
  # Two claims that look alike and are not. vLLM's FP8 documentation puts the
  # weight-only W8A16 floor at 7.5 through FP8 Marlin, and native W8A8
  # arithmetic at 8.9. This table previously started weight-only at 8.0, which
  # told a Turing owner they had no FP8 path when they had the same one Ampere
  # has -- and the error is easy to re-introduce in the other direction, by
  # letting 7.5 read as though it computes in FP8.
  assert_eq "$(vllm_kernels 7.5)" "$(vllm_kernels 8.6)" \
    "Turing and Ampere take the same FP8 path" || return 1

  local out; out="$(vllm_advice 7.5 20000 0)"
  assert_contains "$out" "Turing"     "the sentence names the architecture"   || return 1
  assert_contains "$out" "W8A16"      "and the precision it actually gets"    || return 1
  assert_contains "$out" "no native FP8" "while denying the arithmetic"       || return 1
  assert_not_contains "$out" "no FP8 path at all" \
    "the Volta sentence must not be handed to Turing"
}

test_volta_keeps_the_no_fp8_at_all_verdict_turing_lost() {
  # The other side of that boundary. Moving 7.5 up must not drag 7.0 with it.
  local out; out="$(vllm_advice 7.0 20000 0)"
  assert_contains "$out" "no FP8 path at all" "Volta really has none" || return 1
  assert_not_contains "$out" "W8A16" "and must not be offered the Marlin path"
}

test_the_architecture_name_is_derived_rather_than_hardcoded_per_branch() {
  # The FP8-weight-only branch used to spell its architecture "(Ampere)" in the
  # sentence itself. Correcting the 7.5 boundary turned that string into a lie
  # for every Turing card, so the name is computed now and asserted here.
  assert_eq "$(runtime_arch 6.1)"  "pre-Volta" "Pascal"   || return 1
  assert_eq "$(runtime_arch 7.0)"  "Volta"     "Volta"    || return 1
  assert_eq "$(runtime_arch 7.5)"  "Turing"    "Turing"   || return 1
  assert_eq "$(runtime_arch 8.6)"  "Ampere"    "Ampere"   || return 1
  assert_eq "$(runtime_arch 8.9)"  "Ada"       "Ada"      || return 1
  assert_eq "$(runtime_arch 9.0)"  "Hopper"    "Hopper"   || return 1
  assert_eq "$(runtime_arch 10.0)" "Blackwell" "Blackwell" || return 1
  assert_eq "$(runtime_arch "")"   "unknown"   "unreadable capability" || return 1
  assert_not_contains "$(vllm_advice 7.5 20000 0)" "Ampere" \
    "a Turing card must not be called Ampere"
}
...
test_nvfp4_alone_does_not_earn_the_clear_win_verdict() {
  # TUNING.md's reversal condition is one Blackwell card with >=48 GB, and the
  # code has to mean the same thing the prose does. Kernels were gating this
  # claim by themselves, so a 12 GB Blackwell laptop chip was being told vLLM
  # "pulls clearly ahead" -- on a card that cannot hold the model resident at
  # all, which is the one thing vLLM requires.
  local small large
  small="$(vllm_advice 10.0 12288 0)"
  large="$(vllm_advice 10.0 65536 0)"

  assert_contains "$small" "NVFP4" "the capability is still reported honestly" || return 1
  assert_not_contains "$small" "pulls clearly ahead" \
    "but a card too small to hold a model does not win on kernels" || return 1
  assert_contains "$small" "capacity is the binding one" \
    "and is told which constraint actually binds" || return 1

  assert_contains "$large" "pulls clearly ahead" \
    "capability plus capacity is the case the docs describe"
}

test_the_capacity_gate_is_asserted_on_both_sides_of_48_gb() {
  # Exact boundary, in the units the function is given. An off-by-one here is
  # the difference between hedging and recommending a stack nobody has run.
  local at below
  at="$(vllm_advice 10.0 49152 0)"       # exactly 48 GB
  below="$(vllm_advice 10.0 49151 0)"    # one MB short
  assert_contains "$at" "pulls clearly ahead" "48 GB clears the gate" || return 1
  assert_not_contains "$below" "pulls clearly ahead" "one MB under does not" || return 1
  # And the threshold the prose quotes is the threshold the code applies.
  assert_eq "$VLLM_RESIDENT_WIN_MB" "49152" "48 GB expressed in MB"
}

test_the_capacity_gate_does_not_leak_into_the_other_kernel_classes() {
  # Ampere with a huge budget must still be told it lacks the kernels, and Ada
  # with a small one must not be denied its native-FP8 verdict on capacity --
  # only the Blackwell claim was over-stated, and only it was gated.
  assert_contains "$(vllm_advice 8.6 65536 0)" "no native FP8" \
    "memory cannot conjure kernels" || return 1
  assert_contains "$(vllm_advice 8.9 12288 0)" "native FP8" \
    "and the gate must not silently apply to Ada"
}
...
test_the_advice_says_what_it_is_about() {
  # 00-specs.sh prints this line on its own, with no label around it. Only the
  # two early-return branches used to name vLLM, so on a machine whose RAM does
  # not dwarf its VRAM the report emitted a bare sentence about compute
  # capability and left the reader to guess which runtime it concerned.
  local cc
  for cc in "" 6.1 7.0 7.5 8.0 8.6 8.9 9.0 10.0 12.0; do
    assert_contains "$(vllm_advice "$cc" 20000 8192)" "vLLM" \
      "advice for '${cc:-<empty>}' must name the runtime it is about" || return 1
  done
  return 0
}
